...
def send_local_photo(bot_token, text, image_path):
    ...
    if not subscribers:
        print("Нет подписчиков для отправки уведомления.")
        return
...
def on_failure_callback(context):
    """Функция для упавшей ТАСКИ (достаем task_instance)"""
    bot_token = Variable.get("telegram_secret_token")
    
    # Безопасно извлекаем данные таски
...
def on_success_callback(context):
    """Функция для успешного ДАГА (достаем объект dag)"""
    bot_token = Variable.get("telegram_secret_token")
    
    # На уровне всего дага объекта task_instance нет, берем данные из dag
    dag = context.get('dag')
...
# Добавляем default_args для вызова алерта на каждую упавшую таску
default_args = {
    "on_failure_callback": on_failure_callback
}

@dag(
    default_args=default_args,                 # <-- Путь 2: Применяем ко всем таскам
    dag_id="retail_vault_full_load",
    schedule_interval=None,
    start_date=datetime(2024, 1, 1),
    catchup=False,
    on_success_callback=on_success_callback,   # <-- Оставляем здесь, чтобы получить 1 сообщение об успехе в самом конце
